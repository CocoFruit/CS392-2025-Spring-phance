...
class LinStream<T> {
	...
	public LinStream<T> filter(Predicate<T> pred) {
		...
		while (cxs.consq()) {
			...
			if (pred.test(head)) {
				// The reason the recursion isnt actually called is because of the () ->
				// it gets suspended as a function. 
	...
	public LinStream<T> map(UnaryOperator<T> fopr){
		LinStrcon<T> cxs = value.get();
		while(cxs.consq()){ // while not empty
			final T head = cxs.head;
			final LinStream<T> tail = cxs.tail;
	...
	// public LinStream<T> concat(LinStream<T> a, LinStream<T> b){
		// LinStrcon<T> axs = a.eval();
		// while(axs.consq()){
		// }
		// 
	// }
